...
class AgentCommandValidator:
    def __init__(self, commands: dict[str, Command], client=None, model: str='', syntax:str='JSON',
        ...
        if syntax == 'JSON':
            ...
            self._input_validator = TOMLResponseValidator(AgentThoughtSchemaTOML, 'No valid TOML found. Return valid TOML with reasoning and next command to use.')
    ...
    async def validate_response(self, memory, functions, tokenizer, response, remaining_attempts) -> Union[AgentThought, None]:
        ...
        try:
          ...
          if not('command' in thought) or not('inputs' in thought):
              ...
              return {
                  'type': 'Validation',
                  'valid': False,
                  'feedback': 'command not found or invalid, or inputs missing. The commands you have are {list(self._commands.keys())}'
              }
        ...
        return {
            'type': 'Validation',
            'valid': False,
            'feedback': 'The command '+thought['command']+' inputs were missing or malformed. Provide them in this format: '+command.schema
        }
